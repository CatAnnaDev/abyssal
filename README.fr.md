...
- `obs_overlay` — si activé, écrit un `abyssal.obs.html` auto-rafraîchi (fond transparent, grosses polices) ~4×/s ; à ajouter comme **source Navigateur** OBS (fichier local) pour un overlay de stream propre à côté de la capture du terminal. La carte montre l'héroïne (nom, classe, niveau, trait, origine), les tags de run (état d'esprit, difficulté, vague Boss Rush, mutateurs), des barres PV/XP/corruption en direct (la couleur des PV passe de vert à ambre puis rouge), les stats clés, l'arme/armure équipées avec élément/affixe/bonus de set, les statuts actifs (poison/feu/bouclier/regen/rage, potions, parchemins), le familier & les compagnons, la pensée du moment, un feed d'événements colorés, une bannière de mort avec épitaphe, et une ligne Twitch (canal, viewers, hype, pool de paris, top chat, dernier résultat de pronostic)
